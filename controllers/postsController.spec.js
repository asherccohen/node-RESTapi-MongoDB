...
var Post = require('../models/postModel');
    ...
    describe("Post a new post", function(){
        it("should create new post", function(done){
            var PostMock = sinon.mock(new Post({ body: 'Save new post from mock'}));
        ...
        it("should return error, if post not saved", function(done){
            var PostMock = sinon.mock(new Post({ body: 'Save new todo from mock'}));
            var post = PostMock.object;
  ...
  describe("Update a new post by id", function(){
    ...
    describe("Delete a post by id", function(){
        it("should delete a postc by id", function(done){
            var PostMock = sinon.mock(Post);
            var expectedResult = { status: true };
            PostMock.expects('remove').withArgs({_id: 12345}).yields(null, expectedResult);
            Post.remove({_id: 12345}, function (err, result) {
                PostMock.verify();
                PostMock.restore();
                expect(result.status).to.be.true;
                done();
        ...
        it("should return error if delete action is failed", function(done){
            var PostMock = sinon.mock(Post);
            var expectedResult = { status: false };
            PostMock.expects('remove').withArgs({_id: 12345}).yields(expectedResult, null);
            Post.remove({_id: 12345}, function (err, result) {
                PostMock.verify();
                PostMock.restore();
                expect(err.status).to.not.be.true;
